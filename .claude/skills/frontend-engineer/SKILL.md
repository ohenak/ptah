...
6. **Commit and push.** Stage the cross-review file, commit with `docs({NNN}): add frontend-engineer cross-review of {document-type}`, and push to the remote branch. Verify the push succeeds.
...
```
<task_done>
action: {CREATE_TSPEC | CREATE_PLAN | REVIEW | IMPLEMENT | ADDRESS_REVIEW}
status: {DONE | BLOCKED}
...
- `artifact` is omitted when there is no file output (e.g., implementation tasks reference the PLAN instead).
