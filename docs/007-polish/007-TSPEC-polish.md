...
| # | Question | Resolution |
|---|----------|------------|
| OQ-TSPEC-01 | Where does `AgentRegistry` hot-reload fit within `NodeConfigLoader`'s existing hot-reload? Does the registry rebuild on config file change? | **Confirmed by PM.** The composition root rebuilds the registry and updates the Orchestrator's registry reference on hot-reload. This mirrors Phase 1 config hot-reload behavior. In-flight invocations complete with their snapshot; de-registration for in-flight is out of Phase 7 scope. |
