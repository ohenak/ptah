...
| J4 | Update composition root `bin/ptah.ts`: call `buildAgentRegistry()` from loaded config before constructing `DefaultOrchestrator`; wire `agentRegistry` into `OrchestratorDeps` and `DefaultRoutingEngine` constructor | _(smoke-tested via integration test in Phase K)_ | `bin/ptah.ts` | ✅ Done |
