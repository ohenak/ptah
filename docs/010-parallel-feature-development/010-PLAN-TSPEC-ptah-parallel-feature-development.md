...
| B-02 | **`featureNameToSlug`** — lowercase, collapse non-alphanumeric to single hyphen, strip leading/trailing hyphens, return "unnamed" for empty result; test cases MUST include: normal string, empty string `""`, all-hyphens `"---"`, all-special `"#@!"` (last two must return `"unnamed"` to surface boundary) | `ptah/tests/unit/orchestrator/feature-branch.test.ts` | `ptah/src/orchestrator/feature-branch.ts` | ⬚ Not Started |
...
| D-04 | **`pullInWorktree`** — `git -C {path} pull {remote} {branch}`; catch "couldn't find remote ref" and silently return (new-branch skip path); throw on network/auth/diverged error; test MUST enumerate all 3 cases explicitly: (1) success — branch exists on remote, pull completes normally; (2) new-branch silent return — stderr contains "couldn't find remote ref", function resolves without throwing; (3) genuine error — any other stderr (e.g. network failure simulation) throws through to caller | `ptah/tests/integration/services/git.test.ts` (local bare repo as remote) | `ptah/src/services/git.ts` | ⬚ Not Started |
...
| E-02 | **Two-tier merge: happy path** — after sub-branch commit, acquire lock → `checkoutBranchInWorktree` → `pullInWorktree` → `mergeInWorktree(--no-ff)` → `pushInWorktree` → clean up both worktrees → return `"merged"` | `ptah/tests/unit/orchestrator/artifact-committer.test.ts` (new test: happy path MUST assert: (a) merge worktree created, (b) `pushInWorktree` called with `remote="origin"` AND `branch=featureBranch` (not the sub-branch name — catches REQ-FB-03 violations), (c) both worktrees removed, (d) result is `"merged"`) | `ptah/src/orchestrator/artifact-committer.ts` | ⬚ Not Started |
| E-03 | **Two-tier merge: pull failure** — genuine `pullInWorktree` throw → clean up both worktrees → release lock → return `"pull-error"` | `ptah/tests/unit/orchestrator/artifact-committer.test.ts` (test MUST include a "lock released" assertion: after the error path resolves, a second `mergeLock.acquire` call on the same lock MUST succeed — a leaked lock would cause all subsequent merges to hang at E-07) | `ptah/src/orchestrator/artifact-committer.ts` | ⬚ Not Started |
...
| E-05 | **Two-tier merge: merge-error** — `mergeInWorktree` returns `"merge-error"` → `abortMergeInWorktree` → clean up both worktrees → release lock → return `"merge-error"` | `ptah/tests/unit/orchestrator/artifact-committer.test.ts` (same lock-release assertion as E-03: second `mergeLock.acquire` MUST succeed after merge-error resolves) | `ptah/src/orchestrator/artifact-committer.ts` | ⬚ Not Started |
| E-06 | **Two-tier merge: push failure** — `pushInWorktree` throws → do NOT remove merge worktree → clean up sub-branch → release lock → return `"push-error"` with `retainedWorktreePath` | `ptah/tests/unit/orchestrator/artifact-committer.test.ts` (same lock-release assertion as E-03: second `mergeLock.acquire` MUST succeed after push-error resolves — the retained merge worktree is preserved for manual resolution but the lock must not be leaked) | `ptah/src/orchestrator/artifact-committer.ts` | ⬚ Not Started |
...
| G-09 | **`executePatternBResume` parity** — apply same feature branch setup and naming changes to the pattern B resume path | `ptah/tests/unit/orchestrator/orchestrator.test.ts` (test MUST enumerate these assertions matching G-01/G-04/G-05 coverage: (a) `createBranchFromRef` called with `featureBranch, "main"` when branch does not exist; (b) `createWorktreeFromBranch` called — `createWorktree` NOT called; (c) `featureBranch` present in `InvocationGuardParams` passed to `invokeWithRetry`) | `ptah/src/orchestrator/orchestrator.ts` | ⬚ Not Started |
...
- [ ] REQ-FB-04 (no merge-to-main) is covered **at compile time**: `GitClient` interface has no `merge(target: "main")` method — the two-tier flow only calls `mergeInWorktree` (sub-branch → feature branch) and `pushInWorktree` (feature branch → remote). No explicit runtime negative assertion is needed because the method literally does not exist on the interface. This decision is documented here (Q-01 resolution).
