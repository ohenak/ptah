...
| REQ-MG-02 | Merge conflict handling | If a merge from a sub-branch to the feature branch results in a content conflict, the orchestrator must abort the merge, retain the worktree for manual resolution, and post a Discord notification. Manual resolution is entirely developer-managed (see [A-10-05]): the developer runs `git merge --continue` or `git merge --abort` from the retained worktree path. | WHO: As a developer GIVEN: Agent A's sub-branch modifies `ptah/src/services/git.ts` AND Agent B's sub-branch also modifies `ptah/src/services/git.ts` AND Agent A's merge succeeds first WHEN: Agent B's merge is attempted THEN: The merge is aborted AND the worktree is retained at its path AND a Discord message is posted that includes: (1) the list of conflicting file paths, (2) the absolute path to the retained worktree, (3) the sub-branch name, and (4) the commands the developer should run to resolve: `cd {worktree-path} && git merge --continue` (after editing conflicted files) or `git merge --abort` to discard | P0 | 10 | [US-11] | [REQ-MG-01] |
| REQ-MG-03 | Feature branch update before merge | Before merging an agent's sub-branch, the orchestrator must ensure the feature branch is up to date with the remote (pull latest). This handles the case where another Ptah instance or a human has pushed commits to the feature branch. | **Success path:** WHO: As the orchestrator GIVEN: The remote `feat-parallel-feature-development` has commits not present locally (e.g., from another Ptah instance or manual push) WHEN: An agent's sub-branch merge is attempted THEN: The local feature branch is updated from remote before the merge AND the merge is attempted against the updated feature branch. **Pull failure path:** WHO: As the orchestrator GIVEN: `git pull origin feat-parallel-feature-development` fails (e.g., network error, authentication failure) WHEN: The pre-merge pull is attempted THEN: A Discord message is posted that includes: the feature branch name, the error message from git, and the sub-branch name that could not be merged AND the sub-branch and worktree are cleaned up (the un-merged commit is not lost — it remains in the sub-branch until cleanup) | P1 | 10 | [US-10], [US-11] | [REQ-MG-01] |
