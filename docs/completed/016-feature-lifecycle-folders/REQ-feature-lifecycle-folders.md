...
| ID | Title | Description | Acceptance Criteria | Priority | Phase | Source Scenarios | Dependencies |
|----|-------|-------------|---------------------|----------|-------|-----------------|--------------|
| REQ-PR-01 | Backlog to in-progress promotion | When a feature transitions from backlog to in-progress (triggered when an engineer or tech-lead begins reviewing the REQ document), the feature folder must be moved from `docs/backlog/{feature-slug}/` to `docs/in-progress/{feature-slug}/`. No NNN prefix is assigned. The folder and file names remain unchanged. | WHO: As an engineer GIVEN: A feature exists in `docs/backlog/{feature-slug}/` WHEN: I start any engineer or tech-lead task on this feature THEN: The folder is moved to `docs/in-progress/{feature-slug}/` with no NNN prefix — folder and file names are unchanged | P0 | 1 | [US-02] | [REQ-FS-01], [REQ-FS-02], [REQ-FS-03] |
...
| REQ-PR-03 | NNN assignment uses global max at completion | The NNN assigned during in-progress→completed promotion must be globally unique. It is calculated as `max(NNN across completed/) + 1`, zero-padded to 3 digits. If no numbered folders exist in `completed/`, the first NNN is `001`. | WHO: As a system GIVEN: Multiple features have been completed over time WHEN: A new feature is promoted to completed THEN: Its NNN is one greater than the highest NNN found in the `completed/` folder; WHO: As the Ptah orchestrator GIVEN: `docs/completed/` contains no numbered folders WHEN: The first feature is promoted to completed THEN: Its NNN is `001` | P0 | 1 | [US-03] | [REQ-PR-02] |
| REQ-PR-04 | Internal document references update on promotion to completed | When a feature is promoted to `completed/` and document filenames are NNN-prefixed, any same-folder markdown link that references a renamed file must be updated. **In scope:** markdown link patterns `[text](filename.md)` and `[text](./filename.md)` where the target filename matches a file being renamed in the same feature folder. **Out of scope:** cross-feature references (links pointing to files in other feature folders), bare filename mentions in prose, absolute paths, wiki-style links, reference-style links (`[text][ref]`), and HTML `<a href="">` patterns. These exclusions are intentional — the two in-scope patterns cover all link styles used in this project's documents, and extending to other formats would require a more complex parser with no practical benefit. This scanning and updating is performed as part of the completion promotion activity in the orchestrator. The backlog→in-progress promotion requires no reference updates because no files are renamed. | WHO: As a developer GIVEN: A feature folder has been promoted to completed WHEN: I open any document in the feature folder THEN: All same-folder markdown links in the formats `[text](filename.md)` and `[text](./filename.md)` resolve correctly to the NNN-prefixed filenames | P1 | 1 | [US-03] | [REQ-PR-02], [REQ-SK-08] |
...
| REQ-SK-02 | All skills resolve feature paths across lifecycle folders | All four skills (PM, engineer, tech-lead, test-engineer) must be able to locate a feature folder by its slug via the feature resolver service, which searches `in-progress/`, `backlog/`, and `completed/` directories in that order. For `completed/`, the search must strip the NNN prefix when comparing slugs (e.g., matching slug `temporal-foundation` to folder `015-temporal-foundation`). If the same slug is found in more than one lifecycle folder simultaneously (invariant violation), the resolver must log a warning and return the first match according to search order — it must not throw an error. | WHO: As any Claude skill GIVEN: A feature with slug `{feature-slug}` exists in exactly one lifecycle folder WHEN: The skill needs to read or write artifacts for that feature THEN: The skill resolves the correct full path regardless of which lifecycle folder the feature is in; WHO: As any Claude skill GIVEN: No feature folder with slug `{feature-slug}` exists in any lifecycle folder WHEN: The resolver is called with slug `{feature-slug}` THEN: A not-found signal is returned — no exception is thrown; WHO: As any Claude skill GIVEN: A feature folder with slug `{feature-slug}` exists in both `backlog/` and `in-progress/` simultaneously WHEN: The resolver is called with slug `{feature-slug}` THEN: A warning is logged identifying the collision, and the `in-progress/` result is returned (first match per search order) | P0 | 1 | [US-05] | [REQ-FS-01] |
| REQ-SK-03 | Engineer/tech-lead skill signals backlog→in-progress promotion intent | When the engineer or tech-lead skill begins a review task and detects that the feature is currently in `docs/backlog/`, it signals promotion intent to the Ptah orchestrator. The orchestrator then executes the promotion activity (per [REQ-SK-08]) before invoking the skill. The skill never executes the promotion itself — detection and signalling is its only responsibility here. No NNN prefix is assigned during this promotion. | WHO: As an engineer GIVEN: A feature is in `docs/backlog/{feature-slug}/` WHEN: I start a review task on this feature THEN: The feature is promoted to `docs/in-progress/{feature-slug}/` before my review artifacts are created | P0 | 1 | [US-02] | [REQ-PR-01], [REQ-SK-02], [REQ-SK-08] |
...
| REQ-WT-02 | Orchestrator manages worktree lifecycle | The Ptah orchestrator is responsible for creating and destroying worktrees for each skill invocation. A worktree is created immediately before the skill runs. It is removed after the skill completes (success or failure). On orchestrator startup, a cleanup sweep must identify and remove any dangling worktrees left by a previous crash. Because the in-memory worktree registry does not survive a process crash, the startup sweep must use `git worktree list` to enumerate all worktrees currently attached to the repository, then cross-reference each against live Temporal workflow activity IDs. Any worktree not associated with an active workflow activity must be pruned with `git worktree remove --force`. | WHO: As the Ptah orchestrator GIVEN: A skill completes or fails WHEN: The orchestrator handles the skill's completion THEN: The worktree used by that skill is deleted; WHO: As the Ptah orchestrator GIVEN: The orchestrator process has just started WHEN: Startup initialization runs THEN: `git worktree list` is called, and any worktree path not matched to a live Temporal activity is removed with `git worktree remove --force` | P0 | 1 | [US-06] | — |
...
| REQ-WT-05 | Promotion activities execute inside a fresh worktree | The promotion activities ([REQ-SK-08]) that run `git mv` and commit the result must execute within a newly created worktree, not in the main working tree and not in a worktree reused from a previous run. A fresh worktree guarantees a clean state regardless of whether a prior promotion attempt was interrupted. Idempotency for partial completions is handled by the two-phase folder check in [REQ-NF-02], not by reusing a stale worktree. The worktree is deleted after the promotion activity completes (success or failure). | WHO: As the Ptah orchestrator GIVEN: A backlog→in-progress or in-progress→completed promotion is needed WHEN: The orchestrator runs the promotion activity THEN: A new worktree is created, the promotion (`git mv` + commit + push) is performed within it, and the worktree is deleted on completion | P0 | 1 | [US-02], [US-03], [US-06] | [REQ-SK-08], [REQ-NF-02], [REQ-WT-01], [REQ-WT-02] |
...
| REQ-NF-02 | Idempotent promotion and migration | All promotion operations (backlog→in-progress, in-progress→completed) and the one-time migration must be idempotent. If a feature is already in the target lifecycle folder, the operation skips the folder move. For the completion promotion specifically, a two-phase check is required to handle partial-completion failures: (1) if the destination `completed/{NNN}-{slug}/` folder already exists, skip NNN assignment and the folder move; (2) resume only the file-rename step for any files that have not yet been renamed to their NNN-prefixed names. This prevents NNN re-assignment or duplication if the promotion activity is retried after a partial failure. | WHO: As the Ptah orchestrator GIVEN: A feature folder already exists at `docs/in-progress/{feature-slug}/` WHEN: The backlog→in-progress promotion is triggered again THEN: The folder move is skipped and success is returned — no error is raised and no files are modified; WHO: As the Ptah orchestrator GIVEN: The destination folder `docs/completed/{NNN}-{feature-slug}/` already exists (Phase 1 of completion promotion previously completed) WHEN: The in-progress→completed promotion is triggered again THEN: NNN assignment and the folder move are skipped; the activity proceeds directly to Phase 2 (file renaming); WHO: As the Ptah orchestrator GIVEN: The destination folder `docs/completed/{NNN}-{feature-slug}/` exists and some (but not all) files have been renamed to their NNN-prefixed names WHEN: The completion promotion activity runs THEN: Only files that have not yet received the NNN prefix are renamed; already-renamed files are left unchanged; the final state is identical to a single uninterrupted run | P1 | 1 |
...
| 2.3 | April 3, 2026 | Product Manager | Addressed engineer cross-review of v2.2 (F-01, F-02, Q-01). Fixed REQ-SK-03 description to align with REQ-SK-08 — skill detects and signals intent; orchestrator executes promotion (F-01). Added `git worktree list` + Temporal activity cross-reference as the crash-safe dangling worktree identification mechanism in REQ-WT-02 (F-02). Removed "or reuses" from REQ-WT-05 — promotion always uses a fresh worktree; idempotency is handled by REQ-NF-02 two-phase check (Q-01). |
