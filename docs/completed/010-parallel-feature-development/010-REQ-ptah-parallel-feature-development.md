...
| **Pain points** | Currently, each agent invocation creates an ephemeral worktree on a branch named `ptah/{agentId}/{threadId}/{invocationId}`. These branches are not related to the feature and are destroyed after merge to `main`. There is no way to accumulate parallel agent work on a shared feature branch. |
...
| A-10-05 | Post-conflict resolution is entirely developer-managed in Phase 10. After a conflict is retained, the developer manually runs `git merge --continue` (or `git merge --abort`) and `git push` from the retained worktree. No Ptah automation for conflict re-trigger is provided in this phase. | If developers expect Ptah to assist with conflict resolution, they will be disappointed. Conflict resolution automation is explicitly deferred to a future phase. |
| A-10-06 | The merge-lock (`AsyncMutex`) is in-memory and resets on Ptah restart. If Ptah restarts while agents are queued on the lock, queued merges do not resume automatically — they fail with a timeout. Crash recovery is out of scope for Phase 10. | If Ptah restarts frequently (e.g., hot reload in development), in-flight merges may be lost. Developers should monitor Discord notifications for merge failures after restarts. |
...
| C-10-03 | Git stores refs as filesystem entries: if `feat-parallel-feature-development` exists as a file in `.git/refs/heads/`, git cannot create `feat-parallel-feature-development/eng/a1b2c3d4` because it would require `feat-parallel-feature-development` to be a directory. This produces `fatal: cannot lock ref`. Therefore, **agent sub-branches must NOT use `feat-{name}/` as a prefix.** | Git ref storage design. Sub-branches use the `ptah/{name}/{agentId}/{invocationId}` prefix to avoid this conflict. |
| C-10-04 | The existing SKILL.md files instruct agents to `git checkout` and `git push` feature branches directly. These instructions conflict with the orchestrator-managed worktree model. | SKILL.md files must be updated to remove git workflow instructions that conflict with the orchestrator's branch management. |
...
|----|-------|-------------|---------------------|----------|-------|-----------------|--------------|
| REQ-FB-01 | Feature branch creation | The orchestrator must create a `feat-{feature-name}` branch from `main` on the first agent invocation for a feature, if the branch does not already exist. The feature name is derived from the Discord thread name using the same slug convention as Phase 9 ([REQ-AF-04]). | **Happy path:** WHO: As the orchestrator GIVEN: A new Discord thread "010-parallel-feature-development — create TSPEC" triggers an agent invocation AND no `feat-parallel-feature-development` branch exists WHEN: The orchestrator prepares the invocation THEN: A `feat-parallel-feature-development` branch is created from `main` AND the agent's sub-branch is created from this feature branch. **Slug transformation:** WHO: As the orchestrator GIVEN: A Discord thread named "My Feature #2 — create TSPEC" triggers an agent invocation WHEN: The orchestrator derives the feature name THEN: The branch is named `feat-my-feature-2` (lowercased, special characters replaced with hyphens, consecutive hyphens collapsed) AND the branch is created from `main`. | P0 | 10 | [US-10], [US-11] | [REQ-AF-04] |
...
| REQ-FB-03 | Feature branch push to remote | After merging an agent's sub-branch into the feature branch, the orchestrator must push the feature branch to the remote (`origin`). This ensures the feature branch is available for PR creation and review. | **Success path:** WHO: As the orchestrator GIVEN: An agent's sub-branch has been successfully merged into `feat-parallel-feature-development` WHEN: The merge completes THEN: `feat-parallel-feature-development` is pushed to `origin` AND the remote branch reflects the merged commit. **Push failure path:** WHO: As the orchestrator GIVEN: The `git push origin feat-parallel-feature-development` command is rejected (e.g., remote has advanced due to a concurrent push) WHEN: The push fails THEN: A Discord message is posted that includes: the feature branch name, the error message from git, and the worktree path that contains the un-pushed commit AND the worktree is retained for manual resolution AND the local feature branch commit is preserved | P0 | 10 | [US-10] | [REQ-MG-01] |
...
| ID | Title | Description | Acceptance Criteria | Priority | Phase | Source Scenarios | Dependencies |
|----|-------|-------------|---------------------|----------|-------|-----------------|--------------|
| REQ-AB-01 | Agent sub-branch naming | Each agent invocation must create a worktree on a sub-branch named `ptah/{feature-name}/{agentId}/{invocationId}`. This replaces the current `ptah/{agentId}/{threadId}/{invocationId}` naming. The sub-branch must be unique across all concurrent worktrees. The prefix `ptah/` (not `feat-{name}/`) is required to avoid the git ref file/directory conflict described in [C-10-03]. | WHO: As the orchestrator GIVEN: A backend-engineer agent is invoked on feature "parallel-feature-development" with invocation ID "a1b2c3d4" WHEN: The worktree is created THEN: The branch name is `ptah/parallel-feature-development/eng/a1b2c3d4` AND the worktree is checked out to this branch AND `git branch --list ptah/parallel-feature-development/eng/a1b2c3d4` returns the branch | P0 | 10 | [US-11], [US-12] | [REQ-FB-01] |
...
| REQ-AB-04 | Sub-branch cleanup | After an agent's sub-branch is merged to the feature branch, the sub-branch and its worktree must be cleaned up. If the merge fails due to a technical git error (not a content conflict — see [REQ-MG-02] for content conflicts), the worktree and sub-branch must also be cleaned up. Sub-branches must not accumulate indefinitely. Note: "merge fails" (technical error, e.g., git exits non-zero for reasons other than content conflict) and "merge conflict" (content conflict detected during merge) are treated differently — conflict worktrees are retained per [REQ-MG-02], technical failure worktrees are cleaned up. | **Success path:** WHO: As the orchestrator GIVEN: Agent A's sub-branch `ptah/parallel-feature-development/eng/a1b2c3d4` has been merged to `feat-parallel-feature-development` WHEN: The merge and push complete THEN: The worktree for the sub-branch is removed AND the sub-branch is deleted AND `git worktree list` no longer shows the worktree AND `git branch --list ptah/parallel-feature-development/eng/a1b2c3d4` returns empty. **Technical failure path:** WHO: As the orchestrator GIVEN: Agent A's sub-branch merge fails with a non-conflict git error (e.g., corrupted object, permission error) WHEN: The merge command exits with an error that is not a conflict THEN: The worktree is removed AND the sub-branch is deleted AND a Discord notification is posted with the error details | P0 | 10 | [US-11], [US-12] | [REQ-MG-01] |

### 5.3 Merge Strategy (MG)

| ID | Title | Description | Acceptance Criteria | Priority | Phase | Source Scenarios | Dependencies |
|----|-------|-------------|---------------------|----------|-------|-----------------|--------------|
| REQ-MG-01 | Serialized merge to feature branch | Agent sub-branch merges to the feature branch must be serialized using the existing merge-lock mechanism. The merge target is `feat-{feature-name}` instead of `main`. Only one merge may execute at a time per feature branch. | WHO: As the orchestrator GIVEN: Agent A and Agent B both complete work at approximately the same time WHEN: Both attempt to merge their sub-branches to `feat-parallel-feature-development` THEN: One merge executes first, the other waits for the lock AND both merges succeed sequentially AND `git log --oneline feat-parallel-feature-development` shows commits from both Agent A and Agent B, in the order the lock was acquired AND neither commit is missing or overwritten. For lock timeout behavior, see [REQ-MG-04]. | P0 | 10 | [US-11], [US-12] | [REQ-AB-01], [REQ-FB-01] |
| REQ-MG-02 | Merge conflict handling | If a merge from a sub-branch to the feature branch results in a content conflict, the orchestrator must retain the merge worktree in conflict state (MERGE_HEAD) for manual resolution, and post a Discord notification. The merge is NOT automatically aborted — leaving the worktree in conflict state allows the developer to edit the conflicted files and then run `git merge --continue`. Manual resolution is entirely developer-managed (see [A-10-05]): the developer runs `git merge --continue` (after editing conflicted files) or `git merge --abort` to discard, from the retained worktree path. | WHO: As a developer GIVEN: Agent A's sub-branch modifies `ptah/src/services/git.ts` AND Agent B's sub-branch also modifies `ptah/src/services/git.ts` AND Agent A's merge succeeds first WHEN: Agent B's merge is attempted THEN: The merge is left in conflict state (MERGE_HEAD) AND the worktree is retained at its path AND a Discord message is posted that includes: (1) the list of conflicting file paths, (2) the absolute path to the retained worktree, (3) the sub-branch name, and (4) the commands the developer should run to resolve: `cd {worktree-path} && git merge --continue` (after editing conflicted files) or `git merge --abort` to discard | P0 | 10 | [US-11] | [REQ-MG-01] |
| REQ-MG-03 | Feature branch update before merge | Before merging an agent's sub-branch, the orchestrator must ensure the feature branch is up to date with the remote (pull latest). This handles the case where another Ptah instance or a human has pushed commits to the feature branch. If the feature branch has no remote tracking ref yet (first invocation — branch was created locally and never pushed), the pull step is skipped (treated as "already up to date") and the merge proceeds normally. | **Success path:** WHO: As the orchestrator GIVEN: The remote `feat-parallel-feature-development` has commits not present locally (e.g., from another Ptah instance or manual push) WHEN: An agent's sub-branch merge is attempted THEN: The local feature branch is updated from remote before the merge AND the merge is attempted against the updated feature branch. **New branch path (no remote tracking ref):** WHO: As the orchestrator GIVEN: The feature branch `feat-parallel-feature-development` was just created locally AND has never been pushed to remote (first invocation for this feature) WHEN: The pre-merge pull is attempted THEN: The pull step is skipped (the branch has no remote tracking ref, so there is nothing to pull) AND the merge proceeds using the local feature branch HEAD AND no error is reported. **Pull failure path:** WHO: As the orchestrator GIVEN: `git pull origin feat-parallel-feature-development` fails with a true error (e.g., network error, authentication failure, diverged history) — not a "no remote ref" condition WHEN: The pre-merge pull is attempted THEN: A Discord message is posted that includes: the feature branch name, the error message from git, and the sub-branch name that could not be merged AND the sub-branch and worktree are cleaned up (the un-merged commit is not lost — it remains in the sub-branch until cleanup) | P1 | 10 | [US-10], [US-11] | [REQ-MG-01] |
| REQ-MG-04 | Merge-lock timeout handling | The merge-lock (`AsyncMutex`) has a default acquisition timeout of 10,000ms. If an agent waits more than this duration for the lock (because another merge is in progress), the lock acquisition fails with `MergeLockTimeoutError`. The orchestrator must handle this error explicitly. | WHO: As the orchestrator GIVEN: Agent A holds the merge-lock AND Agent B has been waiting for the lock for more than 10,000ms WHEN: Agent B's lock acquisition times out THEN: A Discord message is posted that includes: the feature branch name, the sub-branch name that timed out, and a message indicating the merge was not performed AND the sub-branch and worktree are cleaned up AND `git log --oneline feat-{feature-name}` does NOT contain a partial or corrupt merge commit from Agent B | P1 | 10 | [US-11], [US-12] | [REQ-MG-01] |

### 5.4 Skill Alignment (SK)

| ID | Title | Description | Acceptance Criteria | Priority | Phase | Source Scenarios | Dependencies |
|----|-------|-------------|---------------------|----------|-------|-----------------|--------------|
| REQ-SK-01 | Remove agent git checkout instructions | SKILL.md files for all agents (PM, BE, FE, QA) currently instruct agents to run `git checkout -b feat-{feature-name}` and `git push origin feat-{feature-name}`. These instructions must be removed or replaced, because the orchestrator manages branch creation and push via worktrees. Agents must not perform their own branch management. | WHO: As a skill author reviewing the updated SKILL.md GIVEN: The SKILL.md has been updated WHEN: I read the Git Workflow section THEN: The file contains no instructions to run `git checkout`, `git branch`, or `git push` AND the file contains an explicit statement that branch management (worktree creation, sub-branch creation, push to remote) is handled by the orchestrator and must not be performed by the agent | P0 | 10 | [US-10] | — |
...
|----|-------|-------------|---------------------|----------|-------|
| REQ-PF-NF-01 | Merge latency | The time added by the two-tier merge (sub-branch → feature branch → push) must not exceed 10 seconds per agent completion, excluding network latency for `git push`. | WHO: As the orchestrator GIVEN: A local bare git repository is used as the remote (to eliminate network variance) AND the sub-branch has commits WHEN: The orchestrator performs the merge and push THEN: The elapsed time from merge-lock acquire to push complete is less than 10 seconds, measured using the local bare repository as `origin` | P1 | 10 |
| REQ-PF-NF-02 | Worktree disk usage | Each agent worktree is a full git checkout. The orchestrator must clean up worktrees promptly after merge to avoid unbounded disk usage. At any given time, no more worktrees should exist than there are active agent invocations, plus any retained for conflict resolution. Conflict-retained worktrees have no TTL and are retained indefinitely until the developer manually removes them — this is expected and documented (see [A-10-05]). | `git worktree list` count equals active invocation count plus the count of explicitly conflict-retained worktrees (those posted to Discord per [REQ-MG-02]) | P1 | 10 |
| REQ-PF-NF-03 | Backward compatibility | Existing single-agent workflows (one agent at a time per feature) must continue to work without configuration changes. The two-tier branch strategy must be the default behavior, not an opt-in feature. | WHO: As a developer running a single-agent workflow GIVEN: Ptah is configured as today (no parallelism settings) AND a single agent invocation completes WHEN: The agent finishes and the orchestrator merges its work THEN: The agent's commits appear on `feat-{feature-name}` AND `origin/feat-{feature-name}` reflects those commits AND `main` is unchanged AND no additional configuration was required | P0 | 10 |
...
- Two-tier branch strategy: `feat-{feature-name}` (persistent) → `ptah/{feature-name}/{agentId}/{invocationId}` (ephemeral per-agent)
...
- Sub-branch and worktree cleanup after merge (success and technical failure)
- SKILL.md updates to remove conflicting git workflow instructions
- Merge conflict detection, worktree retention, and developer notification
- Lock timeout handling with developer notification
...
- Conflict resolution automation (deferred — developer resolves manually per [A-10-05])
- Merge failure retry logic (deferred — single attempt per invocation in Phase 10)
- Merge-lock persistence across Ptah restarts (in-memory lock resets on restart per [A-10-06])
...
| R-10-04 | Merge-lock contention under high parallelism (5+ concurrent agents) causes timeouts | Low | Medium | Default timeout is 10,000ms. Agents exceeding this are notified and cleaned up per [REQ-MG-04]. Monitor lock wait times in orchestrator logs. Consider per-feature locks or increased timeout if contention is observed. | [REQ-MG-01], [REQ-MG-04] |
...
| Priority | Count | IDs |
|----------|-------|-----|
| P0 | 12 | REQ-FB-01, REQ-FB-02, REQ-FB-03, REQ-FB-04, REQ-AB-01, REQ-AB-02, REQ-AB-03, REQ-AB-04, REQ-MG-01, REQ-MG-02, REQ-SK-01, REQ-PF-NF-03 |
| P1 | 5 | REQ-MG-03, REQ-MG-04, REQ-SK-02, REQ-PF-NF-01, REQ-PF-NF-02 |
| P2 | 0 | — |
...
| Phase | Count | IDs |
|-------|-------|-----|
| Phase 10 | 17 | All requirements in this document |
...
| 1.1 | March 14, 2026 | Product Manager | Addressed BE and QA review findings. Critical: sub-branch naming changed from `feat-{name}/{agentId}/{invocationId}` to `ptah/{name}/{agentId}/{invocationId}` to avoid git ref file/directory conflict (C-10-03, REQ-AB-01). Added: lock timeout requirement (REQ-MG-04), error-path ACs to REQ-FB-03 and REQ-MG-03, failure-path THEN clause to REQ-AB-04, observable THEN clauses to REQ-MG-01/REQ-MG-02/REQ-SK-01, slug transformation edge-case AC to REQ-FB-01. Added A-10-05 (manual conflict resolution), A-10-06 (in-memory lock resets on restart). Fixed duplicate §8 heading; renumbered §9–§11. Fixed P0 count (10→12). REQ-PF-NF-01 and REQ-PF-NF-03 reformatted as Who/Given/When/Then. Total: 17 requirements (was 14). |
| 1.2 | March 14, 2026 | Product Manager | Addressed TSPEC cross-review findings F-01 and F-02. REQ-MG-02: corrected contradictory "merge is aborted" language — the merge is now correctly described as retained in conflict state (MERGE_HEAD), not aborted, so the developer can run `git merge --continue`. REQ-MG-03: added "New branch path" acceptance criteria for the case where the feature branch has no remote tracking ref (first invocation) — pull is skipped rather than failing with "couldn't find remote ref". OQ-03 (docs/ filter for code artifacts) is an open product decision pending resolution before ArtifactCommitter implementation. |
