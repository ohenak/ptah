...
| Attribute | Detail |
|-----------|--------|
| **Description** | Before starting a new Temporal workflow, `ptah run` checks for any workflow with status `Running` or `ContinuedAsNew` for the same feature. The existing `listWorkflowsByPrefix("ptah-")` method in `TemporalClientWrapper` (client.ts) must be enhanced to accept an optional `statusFilter?: ("Running" \| "ContinuedAsNew")[]` parameter that appends an `AND ExecutionStatus IN (...)` clause to the Temporal visibility query. `ptah run` calls this method with `statusFilter: ["Running", "ContinuedAsNew"]` and the feature slug appended to the prefix (e.g., `"ptah-my-feature"`). If any workflow is returned, the command prints an error and exits with code 1 without starting a duplicate. Closed and terminated workflows with the same ID do not block a new start. If the Temporal query itself throws an exception (network error, server unreachable), `ptah run` treats this as a hard error: it prints `Error: unable to check for running workflows: <error message>` and exits with code 1. |
| **Acceptance Criteria** | **Who:** Engineer **Given:** A Temporal workflow for `my-feature` is already in `Running` status **When:** They run `ptah run docs/in-progress/my-feature/REQ-my-feature.md` **Then:** The command prints `Error: workflow already running for feature "my-feature". Use --from-phase to restart from a specific phase after terminating the existing workflow.` and exits with code 1. **Given:** No running workflow exists for `my-feature` **When:** They run `ptah run` **Then:** The command proceeds normally. **Given:** The Temporal server is unreachable when `ptah run` checks for running workflows **Then:** The command prints `Error: unable to check for running workflows: <error message>` and exits with code 1. |
| **Priority** | P1 |
| **Phase** | 1 |
| **Source Stories** | US-01 |
| **Dependencies** | REQ-CLI-01 |
...
| Attribute | Detail |
|-----------|--------|
| **Description** | The `SkipCondition` type in `PhaseDefinition` is extended to a discriminated union: `{ field: "config.*"; equals: boolean } \| { field: "artifact.exists"; artifact: string }`. A new Temporal Activity `checkArtifactExists(slug: string, docType: string): Promise<boolean>` performs the filesystem check (reads whether `<docType>-{slug}.md` exists in the feature folder). This Activity is called for each phase with a `skip_if: { field: "artifact.exists" }` condition before the main phase loop begins, and its results are stored in a new `FeatureWorkflowState` field `artifactExists: Record<string, boolean>` (keyed by `docType`). `evaluateSkipCondition()` is updated with the signature `(condition: SkipCondition, featureConfig: FeatureConfig, artifactExists?: Record<string, boolean>): boolean`. For the `artifact.exists` branch it reads `artifactExists[condition.artifact]`. All three existing call sites in `feature-lifecycle.ts` must be updated to pass `state.artifactExists` as the third argument. The `evaluateSkipCondition()` call for `artifact.exists` conditions is only valid after the pre-loop activity has run; if called before (i.e., `artifactExists` is `undefined`), the function throws `new Error("evaluateSkipCondition: artifactExists map not yet populated — checkArtifactExists Activity must run before evaluating artifact.exists conditions")`. |
| **Acceptance Criteria** | **Who:** Workflow engine **Given:** A phase with `skip_if: { field: "artifact.exists", artifact: "REQ" }` **When:** `REQ-my-feature.md` exists in the feature folder **Then:** `checkArtifactExists("my-feature", "REQ")` returns `true`, `state.artifactExists["REQ"]` is `true`, and `evaluateSkipCondition(condition, featureConfig, state.artifactExists)` returns `true`, causing the phase to be skipped. **When:** The file does not exist **Then:** `evaluateSkipCondition()` returns `false` and the phase runs. **When:** `evaluateSkipCondition()` is called with an `artifact.exists` condition and `artifactExists` argument is `undefined` (pre-loop) **Then:** The function throws `new Error("evaluateSkipCondition: artifactExists map not yet populated — checkArtifactExists Activity must run before evaluating artifact.exists conditions")`. |
| **Priority** | P0 |
| **Phase** | 1 |
| **Source Stories** | US-01 |
| **Dependencies** | None |
...
| Attribute | Detail |
|-----------|--------|
| **Description** | `isCompletionReady()` in `feature-lifecycle.ts` is updated to the signature `(signOffs: Record<string, boolean>, workflowConfig: WorkflowConfig): boolean`. It must no longer hard-code `signOffs.qa` and `signOffs.pm` as the required sign-off agents. Instead, it derives the required sign-off agent IDs from the `implementation-review` phase's `reviewers` list in the provided `workflowConfig`. A workflow is completion-ready when all agents listed as reviewers on `implementation-review` have submitted an `approved` or `approved_with_minor_issues` sign-off (i.e., `signOffs[agentId] === true` for every `agentId` in the `reviewers` list). For backward compatibility, if `workflowConfig` has no `implementation-review` phase, the function falls back to the legacy `signOffs.qa && signOffs.pm` check. All existing call sites in `feature-lifecycle.ts` must be updated to pass the loaded `workflowConfig` as the second argument. |
| **Acceptance Criteria** | **Who:** Orchestrator **Given:** A call to `isCompletionReady(signOffs, workflowConfig)` where `workflowConfig.implementation-review.reviewers` is `["pm-review", "te-review"]` **When:** Both `signOffs["pm-review"]` and `signOffs["te-review"]` are `true` **Then:** `isCompletionReady()` returns `true`. **Given:** Only one of the two has signed off **Then:** It returns `false`. **Given:** An old `workflowConfig` with no `implementation-review` phase **When:** Both `signOffs.qa` and `signOffs.pm` are `true` **Then:** It returns `true` (legacy path). |
| **Priority** | P0 |
| **Phase** | 1 |
| **Source Stories** | US-02 |
| **Dependencies** | REQ-WF-02 |
...
| Attribute | Detail |
|-----------|--------|
| **Description** | `runReviewCycle()` in `feature-lifecycle.ts` must pass the current `reviewState.revisionCount + 1` (1-indexed) to both `invokeSkill` (so agents know which file name to write) and `readCrossReviewRecommendation` (so the orchestrator reads the correct version). Additionally, `runReviewCycle()` must update `reviewState.writtenVersions[agentId]` to the dispatched revision count after each reviewer invocation, so the optimizer context can enumerate all prior versions via REQ-CR-07. `reviewState.writtenVersions` is initialized to `{}` (empty object) in `buildInitialWorkflowState()` alongside the existing `ReviewState` fields. It is also included in `buildContinueAsNewPayload()` so the version history is preserved across Temporal `ContinueAsNew` boundaries. |
| **Acceptance Criteria** | **Who:** Orchestrator **Given:** A review phase on its third review round **When:** Reviewer agents are dispatched **Then:** `invokeSkill` input carries `revisionCount: 3`; `readCrossReviewRecommendation` is called with `revisionCount: 3`; `reviewState.writtenVersions[agentId]` is updated to 3 for each dispatched reviewer. **Given:** The first review round begins for a new workflow **Then:** `reviewState.writtenVersions` is `{}` at the start of the round, and is `{ [agentId]: 1 }` for each reviewer after the round completes. **Given:** A workflow undergoes a `ContinueAsNew` transition after round 2 **Then:** The resumed workflow has `reviewState.writtenVersions` equal to the values set before the transition (not reset to `{}`). |
| **Priority** | P0 |
| **Phase** | 1 |
| **Source Stories** | US-03 |
| **Dependencies** | REQ-CR-02, REQ-CR-03, REQ-NF-03, REQ-NF-04 |
...
| Attribute | Detail |
|-----------|--------|
| **Description** | `ptah run` must start the Temporal workflow and stream progress without requiring a Discord bot token or server ID. When the workflow reaches a `ROUTE_TO_USER` state (human-in-the-loop question) and `discord` config is absent, `ptah run` prints the question text to stdout formatted as `[Question] <question-text>` followed by `Answer: ` (with trailing space, no newline) as the stdin prompt, then reads a single line from stdin. The response is sent to the workflow via the signal named `"humanAnswerSignal"`. If stdin is closed or reaches EOF before the engineer provides input (e.g., non-interactive terminal or piped input that ends), `ptah run` sends an empty string as the answer to `"humanAnswerSignal"` and prints `Warning: stdin closed before answer was provided; resuming workflow with empty answer.` to stderr. When `discord` config is present, the question is routed to Discord as normal and stdin is not read. |
| **Acceptance Criteria** | **Who:** Engineer with no Discord config **Given:** A `ptah.config.json` with no `discord` section **When:** They run `ptah run docs/my-feature/REQ-my-feature.md` **Then:** The workflow starts and runs. Progress appears on stdout. No Discord-related error is thrown. **When:** The workflow reaches a `ROUTE_TO_USER` state **Then:** stdout prints `[Question] <question-text>` followed by `Answer: ` (no newline). `ptah run` reads one line from stdin. After input is received, it signals the workflow using the `"humanAnswerSignal"` signal name with the answer text. **When:** stdin is closed before the engineer types an answer **Then:** `ptah run` sends an empty string to `"humanAnswerSignal"` and prints `Warning: stdin closed before answer was provided; resuming workflow with empty answer.` to stderr. |
| **Priority** | P1 |
| **Phase** | 1 |
| **Source Stories** | US-01, US-06 |
| **Dependencies** | REQ-CLI-01 |
...
#### REQ-NF-04: `ReviewState` Type Gains `writtenVersions` Field

| Attribute | Detail |
|-----------|--------|
| **Description** | The `ReviewState` type in `temporal/types.ts` must be extended with a new field `writtenVersions: Record<string, number>` (keyed by agentId, value is the highest revision number dispatched for that agent in the current review phase). This is a type-level change required for REQ-CR-04 and REQ-CR-07 to be implementable. The field is added to the `ReviewState` interface alongside the existing `reviewerStatuses` and `revisionCount` fields. The `buildInitialWorkflowState()` function must initialize `writtenVersions` to `{}`. The `buildContinueAsNewPayload()` function must include `writtenVersions` in the serialized `ReviewState` payload so the value survives Temporal `ContinueAsNew` transitions. |
| **Acceptance Criteria** | **Who:** Type system **Given:** `ReviewState` in `temporal/types.ts` **Then:** The type includes `writtenVersions: Record<string, number>`. **Given:** `buildInitialWorkflowState()` **Then:** The returned state has `reviewStates[phaseId].writtenVersions` initialized to `{}` for each review phase. **Given:** A `ContinueAsNew` payload constructed by `buildContinueAsNewPayload()` with `writtenVersions: { "se-review": 2 }` **Then:** The resumed workflow's `ReviewState` has `writtenVersions: { "se-review": 2 }` (not reset). |
| **Priority** | P0 |
| **Phase** | 1 |
| **Source Stories** | US-03 |
| **Dependencies** | None |
...
| Priority | Count | IDs |
|----------|-------|-----|
| P0 | 18 | REQ-CLI-01, REQ-CLI-02, REQ-CLI-04, REQ-WF-01, REQ-WF-02, REQ-WF-03, REQ-WF-05, REQ-WF-06, REQ-AG-01, REQ-AG-02, REQ-CR-01, REQ-CR-02, REQ-CR-03, REQ-CR-04, REQ-CR-05, REQ-CR-06, REQ-NF-01, REQ-NF-03, REQ-NF-04 |
...
| NF — Non-Functional | 4 | REQ-NF-01 through REQ-NF-04 |
...
| 1.3 | April 22, 2026 | Product Manager | Addressed cross-review feedback iteration 2 from software-engineer (6 findings) and test-engineer (9 findings). Key changes: (1) Specified `evaluateSkipCondition()` function signature — new third parameter `artifactExists?: Record<string, boolean>`, all three call sites updated, and exact thrown `Error` message when called before pre-loop activity (SE v2 F-01, TE v2 F-01). (2) Specified `isCompletionReady()` new signature `(signOffs, workflowConfig)` with full parameter semantics and call-site update requirement (TE v2 F-02). (3) Promoted `ReviewState.writtenVersions` to formal typed requirement REQ-NF-04: type field in `temporal/types.ts`, initialization in `buildInitialWorkflowState()`, preservation in `buildContinueAsNewPayload()` (SE v2 F-02, TE v2 F-04). (4) Specified `listWorkflowsByPrefix()` enhancement in `TemporalClientWrapper`: optional `statusFilter` parameter appending Temporal visibility query clause; Temporal query failure → hard error with `Error: unable to check for running workflows: <message>` exit code 1 (SE v2 F-03, TE v2 F-05). (5) Added auto-detection phase-not-in-config error case to REQ-CLI-05: when derived phase ID absent from loaded config, print error and exit code 1 (TE v2 F-03). (6) Specified stdin prompt format (`[Question] <text>` / `Answer: `), signal name (`"humanAnswerSignal"`), and closed-stdin behavior (empty string + stderr warning) in REQ-NF-02 (TE v2 F-06). Total: 25 requirements (+1 new: REQ-NF-04). |
