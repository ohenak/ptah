...
| Field | Detail |
|-------|--------|
| **Document ID** | PROPERTIES-maes-init |
| **Requirements** | [REQ-IN-01 through REQ-IN-08, REQ-NF-06](../requirements/REQ-MAES.md) |
| **Specifications** | [TSPEC-maes-init](../specifications/TSPEC-maes-init.md) |
...
This properties document catalogs every testable invariant for the `maes init` CLI command — a TypeScript/Node.js command that scaffolds the MAES project structure (directories, template files, config, and Git commit) into an existing Git repository. All properties trace to REQ-IN-01 through REQ-IN-08, REQ-NF-06, and TSPEC-maes-init.md.
...
- maes.config.json generation and content correctness
...
| Input | Count | Source |
|-------|-------|--------|
| Requirements analyzed | 9 | [REQ-IN-01 through REQ-IN-08, REQ-NF-06](../requirements/REQ-MAES.md) |
| Specifications analyzed | 1 | [TSPEC-maes-init](../specifications/TSPEC-maes-init.md) |
| Plan tasks reviewed | 0 | N/A — not yet created |
| Integration boundaries identified | 5 | package.json bin, maes.config.json schema, maes/skills/ paths, docs/ structure, Git working tree |
...
| PROP-IN-04 | InitCommand must auto-detect project name from `path.basename(process.cwd())` for `maes.config.json` | REQ-IN-04, TSPEC §5.2 | Unit | P0 |
...
| PROP-IN-06 | InitCommand must create git commit with message `[MAES] init: scaffolded docs structure` when `created[]` is non-empty | REQ-IN-06, TSPEC §6 step 7 | Unit | P0 |
...
| PROP-IN-13 | InitCommand must exit with error `"Staged changes detected. Please commit or stash them before running 'maes init'."` when `git.hasStagedChanges()` returns true | TSPEC §6 step 2, §8 | Unit | P0 |
...
| PROP-IN-20 | `maes.config.json` must contain all required default fields (`project`, `agents`, `discord`, `orchestrator`, `git`, `docs`) with correct default values: `agents.model` = `"claude-sonnet-4-6"`, `agents.active` = `["pm-agent", "dev-agent", "test-agent"]`, `orchestrator.max_turns_per_thread` = `10`, `orchestrator.retry_attempts` = `3`, `git.commit_prefix` = `"[MAES]"`, `git.auto_commit` = `true` | REQ-IN-04, TSPEC §5.2 | Unit | P0 |
| PROP-IN-21 | `.gitkeep` files must be empty (zero bytes) in `docs/architecture/decisions/`, `docs/architecture/diagrams/`, `maes/templates/` | TSPEC §5.2 | Unit | P0 |
...
| PROP-IN-23 | Skill placeholder files (`maes/skills/pm-agent.md`, `dev-agent.md`, `test-agent.md`) must contain correct placeholder content | REQ-IN-07, TSPEC §5.2 | Unit | P0 |
...
| PROP-IN-44 | `maes.config.json` `agents.skills` paths must match the actual scaffolded skill file paths (e.g., `"./maes/skills/pm-agent.md"` must correspond to a created file) | REQ-IN-07, TSPEC §11 #3 | Unit | P0 |
...
| PROP-IN-36 | InitCommand must print `"✓  Committed: [MAES] init: scaffolded docs structure"` after successful commit | TSPEC §6 step 7 | Unit | P0 |
...
| PROP-IN-39 | `maes.config.json` must not contain any literal secret values; `ANTHROPIC_API_KEY` must not appear in the config file in any form (not as a key, value, or comment) | REQ-NF-06 | Unit | P0 |
...
|---|-----|--------|------|----------------|
| 1 | No property for CLI entry point (`bin/maes.ts`) argument parsing | If `maes init` subcommand routing is wrong, none of the properties fire | Low | Defer to plan augmentation — this is a thin composition root concern. An integration test for the CLI entry point would cover it. |
...
|---------|------|--------|---------|
| 1.0 | March 8, 2026 | Test Engineer | Initial properties document — 42 properties across 8 categories derived from 9 requirements and TSPEC-maes-init (Rev 3) |
